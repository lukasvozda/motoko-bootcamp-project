...
actor Dip721Nft {
	...
	public query func list_nfts(): async [(TokenId, Text)] {
        return Iter.toArray(tokenURIs.entries());
    };

    public query func list_nfts_owners(): async [(TokenId, Principal)] {
        return Iter.toArray(owners.entries());
    };

    public query func list_nfts_colors(): async [(TokenId, Colors)] {
	...
	public shared(msg) func mint(uri : Text, col : Colors) : async Nat {
		tokenPk += 1;
