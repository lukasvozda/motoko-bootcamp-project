...
// @ts-nocheck
  ...
  const listMyTokens = async (pricipal) => {
    ...
    //const myTokens = await nft.listTokens()
    const tokensColors = await nft.listTokenColors();
    //console.log(myTokens)
    //console.log(tokensColors)
    var colors = [];
    // this is very lame, better do it in the backend
    for (let token in myTokens){
      //console.log(token);
      var tokenId = myTokens[token][0];
      for (let c in tokensColors){
        if (tokenId == tokensColors[c][0]){
          colors.push([tokenId, tokensColors[c][1]])
        }
      }
